...
def msd():
    ...
    mass = Body(mass=1000.0, inertia=0.5, position=[0.0, 0.0], velocity=[0.0, 10.0], name="mass")
    ...
    f_sd = PtpForce(iMarker=mk_m_spring, jMarker=mk_g_spring, k=1.0e6, L0=1.5, dc=6500.0)
...
def dcp():
    ...
    fw = Weight()
    model = PlanarMultibodyModel(bodies=[link_1, link_2], joints=[j_gl1, j_l1l2], forces=[fw])
    model.t = 0.0
    return model
...
def cs():
    ...
    fn_mot = Function(type="a", coeff=[np.deg2rad(55.1821), np.deg2rad(20.0), 0.0])
