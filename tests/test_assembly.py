...
class TestRevJointAssembly:
    ...
    def test_single_pendulum(self):
        ...
        B1 = Body(mass=1, inertia=1)
        mk_G = Ground.add_marker([0.0, 0.0])
        mk_B = B1.add_marker([0.5, 0.0])  # pivot at left end of body
        j = RevJoint(iMarker=mk_B, jMarker=mk_G, q0=-np.pi / 2)
        _assemble([B1], [j])
        # Body hangs straight down: orientation should be -pi/2
        np.testing.assert_allclose(B1.orientation, -np.pi / 2, atol=1e-12)
        # joint at origin => B1.position = r_joint - A(p)*sP_child
        A = np.array([[np.cos(-np.pi/2), -np.sin(-np.pi/2)],
                       [np.sin(-np.pi/2),  np.cos(-np.pi/2)]])
        expected_r = np.array([[0.0], [0.0]]) - A @ np.array([[0.5], [0.0]])
        np.testing.assert_allclose(B1.position, expected_r, atol=1e-12)

    def test_double_pendulum(self):
        """Two bodies in series via rev joints."""
        B1 = Body(mass=1, inertia=1)
        B2 = Body(mass=1, inertia=1)
        mk_G = Ground.add_marker([0.0, 0.0])
        mk_B1_top = B1.add_marker([0.0, 0.5])
        mk_B1_bot = B1.add_marker([0.0, -0.5])
        mk_B2_top = B2.add_marker([0.0, 0.5])
        j0 = RevJoint(iMarker=mk_B1_top, jMarker=mk_G, q0=0)
        j1 = RevJoint(iMarker=mk_B2_top, jMarker=mk_B1_bot, q0=0)
        _assemble([B1, B2], [j0, j1])
        np.testing.assert_allclose(B1.orientation, 0.0, atol=1e-12)
        np.testing.assert_allclose(B2.orientation, 0.0, atol=1e-12)
        np.testing.assert_allclose(B1.position, np.array([[0.0], [-0.5]]), atol=1e-12)
        np.testing.assert_allclose(B2.position, np.array([[0.0], [-1.5]]), atol=1e-12)
...
class TestTranJointAssembly:
    ...
    def test_slider(self):
        ...
        j = TranJoint(iMarker=mk_B, jMarker=mk_G, q0=2.0)
        _assemble([B1], [j])
        np.testing.assert_allclose(B1.orientation, 0.0, atol=1e-12)
        np.testing.assert_allclose(B1.position, np.array([[2.0], [0.0]]), atol=1e-12)
...
class TestRevTranJointAssembly:
    ...
    def test_coupler(self):
        ...
        B1 = Body(mass=1, inertia=1, orientation=0.0)
        mk_G = Ground.add_marker([0.0, 0.0])
        mk_B = B1.add_marker([0.0, 0.0])
        j = RevRevJoint(iMarker=mk_B, jMarker=mk_G, L=1.0, q0=np.pi / 2)
        _assemble([B1], [j])
