...
def _marker_global(marker) -> np.ndarray:
    ...
    return (np.array([x, y])
            + rotation_matrix(phi) @ marker.local_position.ravel()[:2])
...
def _draw_body(ax, body, col, triad_L, show_markers):
    ...
    if isinstance(shape, Rectangle):
        ...
        patch.set_transform(
            Affine2D().rotate(phi).translate(x, y) + ax.transData)
        ax.add_patch(patch)
        ...
        patch = MplPolygon(
            shape.vertices.copy(), closed=True,
            facecolor=face, edgecolor=edge, linewidth=1.5,
        )
        patch.set_transform(
            Affine2D().rotate(phi).translate(x, y) + ax.transData)
...
def _draw_marker(ax, mk, triad_L: float) -> None:
    ...
    head_x = (gp[0] + triad_L * np.cos(a),
               gp[1] + triad_L * np.sin(a))
    head_y = (gp[0] - triad_L * np.sin(a),
               gp[1] + triad_L * np.cos(a))
    ax.add_patch(FancyArrowPatch(
        (gp[0], gp[1]), head_x, arrowstyle="-|>",
        mutation_scale=8, color=_TRIAD_C_X,
        linewidth=1.4, shrinkA=0, shrinkB=0, zorder=5))
    ax.add_patch(FancyArrowPatch(
        (gp[0], gp[1]), head_y, arrowstyle="-|>",
        mutation_scale=8, color=_TRIAD_C_Y,
        linewidth=1.4, shrinkA=0, shrinkB=0, zorder=5))
    ax.plot(gp[0], gp[1], marker="o", linestyle="none",
            markersize=3.5,
    ...
    if mk_name:
        ax.annotate(mk_name, xy=(gp[0], gp[1]),
                    xytext=(-2.0, -6.0), textcoords="offset points",
                    fontsize=9.0, color="#1c2033",
...
def _draw_joint(ax, joint, r_joint, sh, rail_hw, gap_y) -> None:
    ...
    if isinstance(joint, RevJoint):
        ...
        patch.set_transform(
            Affine2D().rotate(ang).translate(gp[0], gp[1]) + ax.transData)
        ax.add_patch(patch)
...
def preview_model(model, *,
    ...
    ax.set_title(
        f"Model preview — initial state "
        f"({len(model.Bodies)} bodies, {len(model.Joints)} joints)",
        fontsize=10,
    )
        ...
        if show_joint_legend:
            handles = [
                Line2D([], [], marker="o", linestyle="none",
                       ...
                       markersize=10, label="Revolute Joint"),
                Line2D([], [], marker="s", linestyle="none",
                       markerfacecolor="#00bcd4",
                       markeredgecolor="#006978",
                       markersize=10, label="Translational Joint"),
            ]
            ax.legend(handles=handles, title="Joint types",
