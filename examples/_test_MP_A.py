...
B3 = Body(mass=0.5,  inertia=0.2, position=[0.4528, 0.6862], orientation=5.0019)
...
pt_O3  = B3.add_marker([-0.15,  0.00], theta=0.0)               # [7] + V2 orientation

#%% Joints  (MATLAB J1..4 -> Python j0..j3)
j0 = RevJoint(iMarker=pt_A1, jMarker=pt_A2)                    # A1 <-> A2
j1 = RevJoint(iMarker=pt_Q0, jMarker=pt_Q2)                    # Q0 <-> Q2
j2 = RevJoint(iMarker=pt_O0, jMarker=pt_O3)                    # O0 <-> O3
j3 = TranJoint(iMarker=pt_B1, jMarker=pt_O3)                    # B1pt tran O3
...
def tire_contact():
    ...
    del_y = B1.position[1, 0] - _L0_tire   # y_B1 - contact_radius
    if del_y < 0:
